...
export default class BlacklistDisplay extends React.Component {
   ...
   handleRemoveEstab = id => {
      ...
      this.setState({
         rebuilt: this.state.rebuilt.filter(estab => estab.id !== id + 1),
      });
   };

   buildCards = () =>
      this.state.rebuilt.map(estab => {
         return (
            <BlacklistCard remove={this.handleRemoveEstab} establishment={estab} user={this.props.user} />
         );
      });
            ...
            <div className='main-div-blacklist'>
               <div className='border-box'>
                  ...
                  <div className='display-cards-div'>
                     {this.state.readyToRender ? console.log(this.state.userInfo.blacklists) : null}
